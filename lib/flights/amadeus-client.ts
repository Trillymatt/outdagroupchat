...
export interface FlightLookupResult {
  ...
  departureAirport: string;
  arrivalAirport: string;
  /** Airport-local wall-clock time as `YYYY-MM-DDTHH:mm`, ready for a datetime-local input. */
  departureTime: string | null;
  arrivalTime: string | null;
}

interface AmadeusScheduleTiming {
  qualifier: string;
  value: string;
}
...
interface AmadeusFlightPoint {
  iataCode: string;
  departure?: { timings?: AmadeusScheduleTiming[] };
  arrival?: { timings?: AmadeusScheduleTiming[] };
}
...
/** An Amadeus timing looks like `2024-06-01T07:15+02:00`; keep the airport-local wall clock. */
function toLocalWallClock(value: string | undefined): string | null {
...
/**
 * Look up a single scheduled flight by carrier + number on a given date via
 * Amadeus's Flight Schedules endpoint, so a traveler can autofill their card
 * from just a flight number instead of copying every field by hand.
 */
export async function lookupFlight({
  ...
  let response: Response;
  try {
    response = await fetch(`${AMADEUS_BASE_URL}/v2/schedule/flights?${params.toString()}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
  } catch {
    throw new AmadeusError("Could not reach the flight lookup provider — try again.", 503, true);
  }
  ...
  if (!response.ok) {
    ...
    throw new AmadeusError(
      response.status === 400 ? "Check the flight number and date and try again." : "Flight lookup failed — try again.",
      response.status,
      retryable,
    );
  }

  const body = (await response.json()) as { data?: AmadeusScheduledFlight[] };
  const flight = body.data?.[0];
  if (!flight || flight.flightPoints.length < 2) return null;

  const first = flight.flightPoints[0];
  const last = flight.flightPoints[flight.flightPoints.length - 1];
  ...
  return {
    ...
    departureAirport: first.iataCode,
    arrivalAirport: last.iataCode,
    departureTime: toLocalWallClock(first.departure?.timings?.[0]?.value),
    arrivalTime: toLocalWallClock(last.arrival?.timings?.[0]?.value),
  };
}
