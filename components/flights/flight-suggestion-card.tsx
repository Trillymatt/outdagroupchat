      ...
      <div className="flex items-start justify-between gap-2">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            {option.nonstop && <Badge tone="success">Nonstop</Badge>}
          ...
          {formatTime(option.departure_time) && <p className="text-xs text-ink-soft">Departs {formatTime(option.departure_time)}</p>}
          {option.price != null && <p className="text-sm font-medium text-ink">{currency.format(option.price)}</p>}
        </div>
        {canEdit && (
          <button
            type="button"
